  ...
  if(distStr.length>=4)
  {
    ...
    else if(id>2 && anchorPositioningFlag == 1)
    {
      ...
      for(int i = 0; i < 10 ; i++){
        ...
        if(solution.x > 0.1 && solution.y > 0.1){
          solution_found = true;
          break;
        }
        idistances[0] += 10;
      ...
      if(solution_found)
      {
        ...
        //arc(center[0], center[1], idistances[0], idistances[0], 0, TWO_PI);
        //arc((int)anchorArray[1].x+center[0], (int)anchorArray[1].y+center[1], idistances[1], idistances[1], 0, TWO_PI);
        //arc((int)anchorArray[2].x+center[0], (int)anchorArray[2].y+center[1], idistances[2], idistances[2], 0, TWO_PI);
...
void draw_anchor()
{
  ...
  arc((int)x+center[0], (int)y+center[1], 20, 20, 0, TWO_PI);
  text("A1 ", (int)x+center[0]+20, (int)y+center[1]+20);
  
  x = ancla2_ini[0] * Math.cos(theta) - ancla2_ini[1] * Math.sin(theta);
  y = ancla2_ini[0] * Math.sin(theta) + ancla2_ini[1] * Math.cos(theta);
  
  arc((int)x+center[0], (int)y+center[1], 20, 20, 0, TWO_PI);
  text("A2 ", (int)x+center[0]+20, (int)y+center[1]+20);
...
class Anchor
{
  ...
  int x;
  int y;
  void draw(int fcolor, int cx, int cy)
  {  
    fill(100,100,100);
...
void anchor_positioning(double d01, double d02, double d12)
{
...
class Trilateration
{
  ...
  double mu1=0;
  double mu2=0;
  int sphereline( vec3d p1,  vec3d p2,  vec3d sc, double r)
  {
  ...
  int GetLocation2D(vec3d best_solution, int use4thAnchor, vec3d[] anchorArray, int[] distanceArray)
  {    
